...
useEffect(() => {
    ...
    if (userResponse?.user?.email) {
      ...
      if (error) {
        ...
        if (data?.following) {
          ...
          const followedUsernames = data.following.map((user: { username: string }) => user.username);
          const { data: followedUsers, error: followedUsersError } = await supabase
            .from('Users')
            .select('username, profile_pic, is_live')
            .in('username', followedUsernames);

          if (followedUsersError) {
            console.error(followedUsersError);
          } else {
            setFollowedChannels(followedUsers.map((user) => ({
              id: Math.random(), // Assign a unique id, or adjust accordingly if you have a unique id
              username: user.username,
              profile_pic: user.profile_pic || 'https://cdn.pixabay.com/photo/2023/02/18/11/00/icon-7797704_640.png',
          ...
          const { data: recommended, error: recommendedError } = await supabase
            .from('Users')
            .select('id, username, profile_pic, is_live')
            .not('username', 'in', `(${followedUsernames.map((username: string) => `'${username}'`).join(',')})`);
  ...
  if (isLoggedIn) {
    ...
    const fetchRecommendedChannels = async () => {
      const { data, error } = await supabase.from('Users').select('id, username, profile_pic, is_live');
        ...
        {isLoggedIn ? (
          <>
            <div className="flex flex-col">
              ...
              <h2 className="text-xl font-bold mb-4 mt-4 truncate" style={{ fontSize: 'min(1.5vw, 1rem)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Recommended Channels</h2>
              <ul>
                ...
                <li key={channel.id} className="mb-2 truncate flex items-center w-full">
                  <Link href={`/channel/${channel.username}`}>
                    <div className="flex items-center space-x-2 p-1 w-full rounded hover:bg-gray-700 cursor-pointer">
                      ...
                      />
                      <span className="flex-grow">{channel.username}</span>
                      <span className={`ml-2 text-sm ${channel.is_live ? 'text-red-500' : 'text-gray-500'}`}>
                        {channel.is_live ? 'Live' : 'Offline'}
                      </span>
                    </div>
                  </Link>
